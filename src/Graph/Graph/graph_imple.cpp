...
bool Graph::isTreeEdge(int from, int to){
	for(auto& edge: treeList){
		if((from == edge.data && to == edge.to) || (to == edge.data && from == edge.to))return true;
	}
	return false;
}
...
void Graph::printEdge(){
	...
	for(int i = 0; i < n; i++){
		for(auto& it : edgeList[i]){
			if (i < it.to)
				fout << "\t{ source: " << i << ", target: " << it.to << ", weight: " << it.weight <<", isTree: " << isTreeEdge(i, it.to) << "}," << endl;
